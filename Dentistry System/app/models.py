...
class Area(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加Subsection时间
    is_recommended = db.Column(db.Boolean(), default=0) # 是否推荐
    introduction = db.Column(db.Text)           # Subsection简介
...
class Scenic(db.Model):
    ...
    introduction = db.Column(db.Text)           # Subsection简介
    content = db.Column(db.Text)           # Subsection内容描述
    address = db.Column(db.Text) # Subsection地址
    ...
    travels = db.relationship("Travels", backref='scenic')  # Notes外键关系关联
...
class Travels(db.Model):
    ...
    content = db.Column(db.Text)           # Notes内容
    scenic_id = db.Column(db.Integer, db.ForeignKey('scenic.id'))  # 所属Subsection
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
